...
step that produces it — never guessed or reused from a previous run.

## Checklist

0. S0 guide read: call `get_authoring_guide` (no arguments) and read the
   served guide. STOP if it errors.
1. S1 draft: call `create_draft`; extract `catalog_id` and `draft_id`. STOP
   if either is empty.
2. S2 upload: `wc -c` each source file, call `create_draft_source_upload`
   with the file list, PUT each file to its `upload_targets` URL with
   `required_headers` verbatim, then `finalize_draft_source_upload`. STOP if
   any PUT is not 200.
3. S3 required source files: call `get_draft`; all four `required_source_files`
   (connector.ts, config-schema.json, runtime-schema.json, capabilities.json)
   must be true. STOP if any is false.
4. S4 build: call `build_bundle`; extract `run_id`. STOP if empty.
5. S5 build result: poll `get_run` with `run_id` until terminal. GATE:
   `RUN_STATE_SUCCEEDED`; extract the immutable `revision_id`. STOP on
   failure — fix source and re-run from S2 with a fresh `run_id`.
6. S6 app: call `apps_create`; extract `app_id`. STOP if empty.
7. S7 provision: call `provision_connector` under the app; extract
   `connector_id`. STOP if empty.
8. S8 credentials: configure base-url, account-email, and api-token (see
   deploy-and-activate). STOP if any credential is missing.
9. S9 draft test: call `run_draft_test_sync`; extract `test_run_id`. STOP if
   empty.
10. S10 evidence: poll `get_test_run_evidence` with `(catalog_id, revision_id,
    test_run_id)` until the durable row exists. GATE: `result == "PASS"`.
    STOP if FAIL — fix source and re-run from S2 with a FRESH `test_run_id`.
11. S11 handoff discipline: call `deploy_connector_instance` (extract
    `deployment_instance_id`), then `mint_approval_token` (extract
    `activation_url`), then write the handoff table and STOP. STOP if either
    ID is empty.
