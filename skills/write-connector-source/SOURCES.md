...
| Lifecycle doc | c1 `92f1c2e54ec6ca1371c988e580a8cd4865a4c283` | The source-file contract section; the dual config schema section; the common-failures table; the Okta worked example; the transport-registration rule (every transport referenced by a `node`/`reuse` must be the same object registered under `transports:`). |
...
| `author-js-dsl-connector` (baton-admin) | baton-admin `6fe6886f607ed0d2e48a616c30e7ce4bffc32489` | The TypeScript DSL rules named by plan decision 7d (slot identity by JS reference, transport registration); the applied rules are quoted from the investigation §3.4 trip hazards and the lifecycle doc. |
| `runtime-gotchas.md` | claude-marketplace `0cc5ac2a2dbe60b430444c59e53016da2c72b3d1` | The 12 traps (opaque config refs at module eval, ES5 bundle target, scope list calls, pagination termination). |
...
| Investigation | `/shared/src-hv96/investigation.md` | §3.1 source-file contract, §3.4 authoring trip hazards (slot identity by JS reference - never re-call `slot()`), §5 skill 5, §8 build order step 4. |
