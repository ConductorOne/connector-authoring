...
//   [--driver <name>] [--out <dir>] [--max-agent-minutes <n>]
...
// Driver registry (D12): resolution is proven by the subprocess test in
// driver.test.ts — not exported (importing run.ts would execute main()).
...
function parseArgs(args: string[]): CliArgs {
  ...
  // --ref is driver-interpreted (Tier-0 ignores it); restrict to a safe
...
// The run channel is runner-owned local files (decision 4): the driver
// fills handoffInstructions/completionInstructions before the agent runs.
...
async function main(): Promise<number> {
  ...
  // Provision + readiness retry loop (decision 5, D13): the driver's
  // checkReadiness proves tenant reachability; the runner then verifies the
  // scenario's readiness tools are present in the declared tool surface.
  // Failure -> teardown, retry with a fresh provision (max 2 retries).
  ...
  try {
    ...
    // the run channel as a tool result — strip characters that could inject
    ...
    writeFileSync(sanitizedHandoffPath, JSON.stringify(sanitizedHandoff), "utf8")
    ...
    try {
      ...
      const raw = readFileSync(channel.scoreInputPath, "utf8")
      const normalized = normalizeScoreInput(JSON.parse(raw) as unknown)
    ...
    // Run meta: harness = driver name; reasoning_effort is n/a (Tier-0 has
    // no reasoning-effort knob; the field stays for schema byte-compatibility).
    const meta: RunMeta = {
      run_id: runId,
    ...
    // Teardown on EVERY exit path (success or error) — best-effort.
    if (handle) {
      try {
        await driver.provisioner.teardown(handle)
      } catch {
