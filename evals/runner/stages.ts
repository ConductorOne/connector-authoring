...
// driver.write_file to the exact handoff path (args.path, exact equality).
function isHandoffWrite(call: ToolCallRecord, handoffPath: string): boolean {
  if (call.name !== "driver.write_file") return false
  if (typeof call.args !== "object" || call.args === null) return false
  return "path" in call.args && call.args.path === handoffPath
}
